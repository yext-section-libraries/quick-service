...
type PhoneFieldSet = {
  subheading: YextEntityField<TranslatableString>;
  entityPhone: YextEntityField<string>;
  phoneFormat: "domestic" | "international";
  includePhoneHyperlink: boolean;
  fontColor: ThemeColor | undefined;
};

type ImageConstantValue = AssetImageType;

type SectionImageField = {
  image: YextEntityField<ImageConstantValue>;
  aspectRatio: number;
...
.quick-service-details-section .split-banner-wrap {
...
@media (max-width: 1100px) {
  ...
  .quick-service-details-section .split-banner-image {
    display: none;
  }
  ...
  .quick-service-details-section .split-banner-wrap { grid-template-columns: 1fr; }
...
const normalizeTextListValues = (value: unknown): string[] => {
  ...
  if (Array.isArray(value)) {
    return value.flatMap((item) => normalizeTextListValues(item));
  }
  ...
  if (
    ...
    return normalizeTextListValues(
...
const resolveTextListValues = (
  ...
  const resolvedValue = field
    ...
    : undefined;
  const resolvedItems = normalizeTextListValues(resolvedValue);
  if (resolvedItems.length > 0) {
    return resolvedItems;
  }

  return normalizeTextListValues(field?.constantValue);
};
...
const getEntityAddressValues = (streamDocument: any) => {
  const address = streamDocument?.address as AddressType | undefined;

  return {
    line1: toRenderableText(address?.line1).trim(),
    line2: [address?.line2, address?.line3]
      .map((value) => toRenderableText(value).trim())
      .filter(Boolean)
      .join(", "),
    city: toRenderableText(address?.city).trim(),
    state: toRenderableText(address?.region).trim(),
    zipCode: toRenderableText(address?.postalCode).trim(),
  };
};

const defaultPhone: QuickServiceDetailsProps["phone"] = {
  subheading: {
    field: "",
    constantValue: {
      defaultValue: "Phone",
...
const defaultWebsiteLink: Partial<ComprehensiveCTAValue> = {
  data: {
    actionType: "link",
    cta: {
      field: "",
...
const defaultDirectionsLink: Partial<ComprehensiveCTAValue> = {
  data: {
    actionType: "link",
    cta: {
      field: "",
...
const defaultOtherDetails = {
  ...
  content: {
    list: {
      ...
      constantValue: [
        "Price range: Lorem $$",
        "Cuisine: Lorem ipsum, dolor sit amet",
        "Meals served: Lorem brunch, lunch, dinner",
      ],
      constantValueEnabled: true,
    },
    commaSeparated: false,
  },
} satisfies QuickServiceDetailsProps["otherDetails"];

const defaultSectionImage: QuickServiceDetailsProps["sectionImage"] = {
  image: {
    field: "",
    constantValue: {
      url: detailsImageUrl,
...
const isImageConstantValue = (value: unknown): value is ImageConstantValue => {
  return value !== null && typeof value === "object" && "url" in value;
};

const resolveImageFieldValue = (
  value: unknown,
): ImageConstantValue | undefined =>
  isImageConstantValue(value) ? value : undefined;
...
const defaultHeading: QuickServiceDetailsProps["heading"] = {
  text: {
    field: "",
    constantValue: "Restaurant Details",
    constantValueEnabled: true,
  },
  styles: {
    fontFamily: "default",
...
const defaultAddress: QuickServiceDetailsProps["address"] = {
  subheading: {
    field: "",
    constantValue: {
...
const QuickServiceDetailsComponent: PuckComponent<QuickServiceDetailsProps> = (
  props,
) => {
  ...
  const entityPhoneValue = resolveTextFieldValue(
    ...
    locale,
    streamDocument,
    toRenderableText(streamDocument?.mainPhone, ""),
  );
  const phoneValue = entityPhoneValue;
  const phoneColor = getThemeColorCssValue(props.phone.fontColor);
  const otherDetailsColor = getThemeColorCssValue(props.otherDetails.fontColor);
  const otherDetailItems = resolveTextListValues(
    props.otherDetails.content.list,
    locale,
    streamDocument,
  ).filter((item) => item.trim().length > 0);
  const selectedSectionImage = resolveImageFieldValue(
    resolveComponentData(props.sectionImage.image, locale, streamDocument),
  );
  const hasSectionImage =
  ...
  return (
    <AnalyticsScopeProvider
      name={`QuickServiceDetails${getAnalyticsScopeHash(props.id)}`}
    >
      <VisibilityWrapper
      ...
      >
        <>
          <style>{QuickServiceDetailsStyles}</style>
          <Background
            as="section"
            background={props.section.backgroundColor}
            className="split-banner quick-service-details-section"
            style={{
              ...sectionSurfaceStyle,
              overflow: "visible",
            }}
          >
            <div
              className="split-banner-wrap"
              style={
                hasSectionImage ? undefined : { gridTemplateColumns: "1fr" }
              }
            >
              <article
                className="split-banner-text quick-service-details-content"
                style={{ minHeight: "max-content", alignSelf: "start" }}
              >
                <EntityField
                  displayName="Heading"
              ...
              >
                ...
                >
                  ...
                  {phoneValue ? (
                    <EntityField
                      displayName="Phone"
                      fieldId={props.phone.entityPhone.field}
                      constantValueEnabled={
                ...
                <div
                  ...
                  style={{ borderBottomColor: "currentColor" }}
                >
                  <div className="quick-service-detail-ctas">
                    <EntityField
                ...
                >
                  <EntityField
                    ...
                    fieldId={props.otherDetails.subheading.field}
                    constantValueEnabled={
                ...
                >
                  ...
                  <EntityField
                    ...
                    fieldId={props.otherDetails.content.list.field}
                    constantValueEnabled={
...
export const QuickServiceDetails: YextComponentConfig<QuickServiceDetailsProps> =
  {
    ...
    defaultProps: {
      ...
      phone: defaultPhone,
      websiteLink: defaultWebsiteLink,
      directionsLink: defaultDirectionsLink,
      otherDetails: defaultOtherDetails,
      sectionImage: defaultSectionImage,
    },
    render: (props) => <QuickServiceDetailsComponent {...props} />,
  };
