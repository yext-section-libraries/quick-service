...
type QuickServiceFaqProps = {
  section: {
    backgroundColor: ThemeColor;
    faqBackgroundColor: ThemeColor;
    visibleOnLivePage: boolean;
  };
  heading: StyledPlainTextProps;
  faqs: {
    list: typeof faqItemsSource.value;
    styles: {
      question: StyledTextValue;
      answer: StyledTextValue;
    };
  };
};
  ...
  defaultValues: [
    {
      question: {
        field: "",
        constantValue: "What time do you open?",
        constantValueEnabled: true,
      },
      answer: {
        field: "",
        constantValue: {
          defaultValue: getDefaultRTF("We open at 11 AM every day."),
        },
        constantValueEnabled: true,
      },
    },
    {
      question: {
        field: "",
        constantValue: "Do you take reservations?",
        constantValueEnabled: true,
      },
      answer: {
        field: "",
        constantValue: {
          defaultValue: getDefaultRTF("Yes, reservations are available."),
        },
        constantValueEnabled: true,
      },
    },
    {
      question: {
        field: "",
        constantValue: "Is outdoor seating available?",
        constantValueEnabled: true,
      },
      answer: {
        field: "",
        constantValue: {
          defaultValue: getDefaultRTF(
            "We offer both indoor and outdoor seating when the weather allows.",
          ),
        },
        constantValueEnabled: true,
      },
...
const defaultHeading: QuickServiceFaqProps["heading"] = {
  data: {
    text: {
      field: "",
      constantValue: "FAQs",
...
const defaultFaqs: QuickServiceFaqProps["faqs"] = {
  list: faqItemsSource.defaultValue,
  styles: {
    question: {
      fontFamily: "default",
      fontSize: "default",
...
const QuickServiceFaqStyles = String.raw`
...
const defaultFaqProps: QuickServiceFaqProps = {
  section: {
    backgroundColor: {
      selectedColor: "white",
...
const QuickServiceFaqComponent: PuckComponent<QuickServiceFaqProps> = (
  props,
) => {
  ...
  const faqSurfaceStyle = getSurfaceColorStyle(
    props.section.faqBackgroundColor,
    streamDocument,
  );
  const faqs = faqItemsSource.resolveItems(props.faqs.list, streamDocument);
  const questionStyle: React.CSSProperties = {
    fontFamily:
      props.faqs.styles.question.fontFamily === "default"
        ? undefined
        : props.faqs.styles.question.fontFamily,
    fontSize:
      props.faqs.styles.question.fontSize === "default"
        ? undefined
        : props.faqs.styles.question.fontSize,
    fontWeight:
      props.faqs.styles.question.fontWeight === "default"
        ? undefined
        : props.faqs.styles.question.fontWeight,
    fontStyle:
      props.faqs.styles.question.fontStyle === "default"
        ? undefined
        : props.faqs.styles.question.fontStyle,
    textTransform:
      props.faqs.styles.question.textTransform === "default"
        ? undefined
        : props.faqs.styles.question.textTransform,
    color:
      getThemeColorCssValue(props.faqs.styles.question.color) ??
  ...
  > = {
    ...
    color:
      props.faqs.styles.answer.color ??
      props.section.faqBackgroundColor.contrastingColor,
  };

  return (
    <AnalyticsScopeProvider
      name={`QuickServiceFaq${getAnalyticsScopeHash(props.id)}`}
    >
      <VisibilityWrapper
      ...
      >
        <>
          <style>{QuickServiceFaqStyles}</style>
          <Background
            as="section"
            background={props.section.backgroundColor}
            className="quick-service-faq-section"
            aria-label="FAQs"
            style={sectionSurfaceStyle}
          >
            <div className="quick-service-faq-wrap">
              <div className="split-banner-heading quick-service-faq-heading">
                <EntityField
                  displayName="Heading"
              ...
              <EntityField
                displayName="FAQs"
                fieldId={props.faqs.list.field}
                constantValueEnabled={props.faqs.list.constantValueEnabled}
              >
                <div className="quick-service-faq-list">
                  {faqs.map((faq, index) => {
                    ...
                    return (
                      <Background
                        as="div"
                        background={props.section.faqBackgroundColor}
                        key={`faq-${index}`}
                        className={`quick-service-faq-item${isOpen ? " is-open" : ""}`}
                        style={{
                          ...faqSurfaceStyle,
                      ...
                      >
                        <button
                          className="quick-service-faq-trigger"
                          type="button"
                          aria-expanded={isOpen}
                          onClick={() => setOpenIndex(isOpen ? -1 : index)}
                          style={{ color: faqSurfaceStyle?.color }}
                        >
                          <span
                            className="quick-service-faq-label"
                            style={questionStyle}
                          >
                            {question}
                          </span>
                          <span
                            className="quick-service-faq-glyph"
                            aria-hidden="true"
                            style={{
                              color: faqSurfaceStyle?.color,
                              display: "inline-flex",
                              alignItems: "center",
