...
type FeaturedItemSourceItem = {
  ...
  button: YextEntityField<EnhancedTranslatableCTA>;
};

type QuickServiceFeaturedProps = {
  section: {
  ...
  content: {
    list: typeof featuredItemsSource.value;
    styles: {
      description: StyledTextValue;
...
.quick-service-featured-layout {
  display: grid;
  grid-template-columns: minmax(0, .75fr) minmax(0, 3fr);
}
.quick-service-featured-sidebar {
  display: flex;
  flex-direction: column;
}
...
.quick-service-featured-spacer {
  flex: 1;
  background: inherit;
  border-right: 2px solid var(--bc-border);
  border-bottom: 2px solid var(--bc-border);
}
.quick-service-featured-cards {
  display: grid;
...
.quick-service-featured-item-card:nth-child(3n) {
  border-right: 0;
}
.quick-service-featured-item-image-wrap {
  width: 100%;
  overflow: hidden;
  height: 260px;
}
.quick-service-featured-item-image-wrap img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: block;
...
const featuredItemsSource = createItemSource<FeaturedItemSourceItem>({
  ...
  defaultValues: [
    {
      image: buildDefaultImage(featuredItem1ImageUrl),
      title: {
        field: "",
        constantValue: "Lorem Signature Burger",
        constantValueEnabled: true,
      },
      description: {
        field: "",
        constantValue: {
          defaultValue: getDefaultRTF(
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi mollis turpis sit amet sapien cursus, vitae consequat dui volutpat.",
          ),
        },
        constantValueEnabled: true,
      },
      button: defaultFeaturedCta("Learn More"),
    },
    {
      image: buildDefaultImage(featuredItem2ImageUrl),
      title: {
        field: "",
        constantValue: "Ipsum Crispy Sandwich",
        constantValueEnabled: true,
      },
      description: {
        field: "",
    ...
    {
      image: buildDefaultImage(featuredItem3ImageUrl),
      title: {
        field: "",
        constantValue: "Dolor Steak Salad",
        constantValueEnabled: true,
      },
      description: {
        field: "",
        constantValue: {
          defaultValue: getDefaultRTF(
            "Quis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Integer a lorem sed urna.",
          ),
        },
        constantValueEnabled: true,
      },
...
const defaultHeading: QuickServiceFeaturedProps["heading"] = {
  text: {
    field: "",
    constantValue: "Featured Items",
    constantValueEnabled: true,
  },
  styles: {
    fontFamily: "default",
...
const defaultContent: QuickServiceFeaturedProps["content"] = {
  list: featuredItemsSource.defaultValue,
  styles: {
    description: {
      fontFamily: "default",
      fontSize: "default",
...
const QuickServiceFeaturedComponent: PuckComponent<
  QuickServiceFeaturedProps
> = (props) => {
  ...
  const resolvedHeadingText = resolveStringEntityFieldValue(
    ...
    locale,
    streamDocument,
    typeof defaultHeading.text.constantValue === "string"
      ? defaultHeading.text.constantValue
      : "",
  );
  const headingStyles = resolveStyledTextStyles(props.heading.styles);
  const headingSurfaceStyle = getSurfaceColorStyle(
    props.heading.backgroundColor,
    streamDocument,
  );
  const headingColor = getThemeColorCssValue(props.heading.fontColor);
  const contentStyles = resolveStyledTextStyles(
    props.content.styles.description,
  );
  const contentColor = getThemeColorCssValue(
    props.content.styles.description.color,
  );
  const itemHeaderStyles = resolveStyledTextStyles(props.content.styles.title);
  const itemHeaderColor = getThemeColorCssValue(
    props.content.styles.title.color,
  );
  const richTextStyleOverrides = resolveRichTextStyleOverrides(
    props.content.styles.description,
    contentColor,
  );
  const resolvedContentItems = featuredItemsSource
    .resolveItems(props.content.list, streamDocument)
    .map((card) => {
  ...
  return (
    <AnalyticsScopeProvider
      name={`QuickServiceFeatured${getAnalyticsScopeHash(props.id)}`}
    >
      <VisibilityWrapper
      ...
      >
        <>
          <style>{QuickServiceFeaturedStyles}</style>
          <Background
            as="section"
            background={props.section.backgroundColor}
            className="quick-service-featured-section"
            aria-label="Featured Items"
            style={sectionSurfaceStyle}
          >
            <div className="quick-service-featured-layout">
              <div className="quick-service-featured-sidebar">
                <Background
                  as="div"
                  background={props.heading.backgroundColor}
                  className="quick-service-featured-title-card"
                  style={headingSurfaceStyle}
                >
                  <EntityField
                    displayName="Heading"
                  ...
                  >
                    <h2
                      className="quick-service-featured-title"
                      style={{
                        ...headingStyles,
                ...
                <div
                  aria-hidden="true"
                  className="quick-service-featured-spacer"
                  style={sectionSurfaceStyle}
                />
              </div>
              <EntityField
                displayName="Featured Items"
                fieldId={props.content.list.field}
                constantValueEnabled={props.content.list.constantValueEnabled}
              >
                <div className="quick-service-featured-cards">
                  {resolvedContentItems.map((card, index) => (
                    <article
                      key={`${card.title}-${index}`}
                      className="quick-service-featured-item-card"
                      style={sectionSurfaceStyle}
                    >
                      {card.image ? (
                        <div
                          className="quick-service-featured-item-image-wrap"
                          style={{
                            ...sectionSurfaceStyle,
                            aspectRatio:
                              props.content.styles.image.aspectRatio > 0
                                ? props.content.styles.image.aspectRatio
                                : undefined,
                          }}
                        >
                          <Image
                            image={card.image}
                            className="quick-service-featured-item-image"
                            style={{
                              display: "block",
                              width: "100%",
                              height: "100%",
                              objectFit:
                                props.content.styles.image.imageConstrain ===
                                "filled"
                                  ? "cover"
                                  : "contain",
                            }}
                      ...
                      <div
                        className="quick-service-featured-copy"
                        style={{
                          ...contentStyles,
                      ...
                      >
                        ...
                        <CTA
                          ...
                          link={normalizeNavigableUrl(card.button?.link || "#")}
                          linkType="URL"
                          variant="primary"
...
export const QuickServiceFeatured: YextComponentConfig<QuickServiceFeaturedProps> =
  {
    ...
    render: (props) => <QuickServiceFeaturedComponent {...props} />,
  };
