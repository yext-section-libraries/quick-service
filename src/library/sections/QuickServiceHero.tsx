...
type DayData = {
  isClosed?: boolean;
  openIntervals?: IntervalData[];
};

type HeroTextFieldValue = {
  text: YextEntityField<TranslatableString>;
  styles: StyledTextValue;
  fontColor?: ThemeColor;
};

type HeroImageFieldValue = {
...
type HoursDataValue = HoursType;

type QuickServiceHeroProps = {
  section: {
  ...
  brand: {
    name: HeroTextFieldValue;
    logo: HeroImageFieldValue;
  };
  background: {
    image: HeroImageFieldValue;
  };
  hours: {
    entityHours: YextEntityField<HoursDataValue>;
    statusPillColor: ThemeColor;
    hoursStyles: {
  ...
  geomodifier: HeroTextFieldValue;
  ctas: HeroCtaRow[];
};
...
.hero-cta--solid {
  box-shadow: 6px 6px 0 var(--bc-shadow);
  transition: transform .1s .1s cubic-bezier(.19,1,.22,1), box-shadow .1s .1s cubic-bezier(.19,1,.22,1);
}

.hero-cta--solid:hover,
.hero-cta--solid:focus-visible {
...
const defaultHeroCta = (label: string): Partial<ComprehensiveCTAValue> => ({
  data: {
    actionType: "link",
    cta: {
      field: "",
...
const defaultHeroCtas: HeroCtaRow[] = [
  { cta: defaultHeroCta("Call Ahead") },
  { cta: defaultHeroCta("Order Takeout") },
  { cta: defaultHeroCta("View Menu") },
];
...
const toRenderableText = (value: unknown, fallback = "") => {
  ...
  if (value && typeof value === "object") {
    const maybeDayValue = normalizeDayValue(value as DayData);
    if (maybeDayValue) {
      return maybeDayValue;
    }
...
const defaultHeroProps: QuickServiceHeroProps = {
  section: {
    backgroundColor: {
    ...
    image: {
      image: {
        ...
        constantValue: {
          ...
          width: 1900,
          height: 1267,
          alternateText: defaultHeroAltText,
        },
        constantValueEnabled: true,
      },
      aspectRatio: 0,
...
const normalizeDayValue = (value?: string | DayData) => {
...
) => toRenderableText(resolveComponentData(field, locale, streamDocument), "");

const resolveHoursFieldValue = (
  field: YextEntityField<HoursDataValue>,
  locale: string,
  streamDocument: Record<string, any> | undefined,
) => {
  const resolved = resolveYextEntityField(streamDocument, field, locale);
  if (
    resolved &&
...
const QuickServiceHeroComponent: PuckComponent<QuickServiceHeroProps> = (
  ...
  const brandNameColor = getThemeColorCssValue(props.brand.name.fontColor);
  const brandNameStyles = props.brand.name.styles;
  const resolvedBrandLogo = resolveComponentData<
    ImageType | ComplexImageType | TranslatableAssetImage
  >(props.brand.logo.image, locale, streamDocument);
  ...
  const geomodifier = resolveTextFieldValue(
    props.geomodifier.text,
    locale,
    streamDocument,
  );
  const geomodifierColor = getThemeColorCssValue(props.geomodifier.fontColor);
  const geomodifierStyles = props.geomodifier.styles;
  const backgroundImage = resolveComponentData<
    ImageType | ComplexImageType | TranslatableAssetImage
  >(props.background.image.image, locale, streamDocument);
  const statusPillStyle = getSurfaceColorStyle(
    props.hours.statusPillColor,
    streamDocument,
  );
  ...
  const hasBrandLogo =
    typeof resolvedBrandLogo === "object" &&
    resolvedBrandLogo !== null &&
    "url" in resolvedBrandLogo &&
  ...
  return (
    <AnalyticsScopeProvider
      name={`QuickServiceHero${getAnalyticsScopeHash(props.id)}`}
    >
      <VisibilityWrapper
      ...
      >
        <>
          <style>{QuickServiceHeroStyles}</style>
          <Background
            as="section"
            background={props.section.backgroundColor}
            className={hasBackgroundImage ? "hero hero-has-image" : "hero"}
            style={{
              ...sectionSurfaceStyle,
            }}
          >
            ...
            <div className="hero-overlay">
              <Background
                ...
                background={props.section.cardColor}
                style={{
                  ...cardSurfaceStyle,
                }}
              >
                ...
                {resolvedHours ? (
                  ...
                  <div className="hero-status">
                      <HoursStatus
                        ...
                        dayOptions={{
                          weekday:
                            props.hours.hoursStyles.dayOfWeekFormat === "short"
                              ? "short"
                              : "long",
                        }}
                        currentTemplate={
              ...
              <div className="hero-cta-row">
                {ctaItems.map((item, index) => {
                  const ctaVariant = item.styles?.variant ?? "primary";
