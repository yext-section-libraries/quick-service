...
type HoursSectionImageField = {
  image: YextEntityField<ImageType | ComplexImageType | TranslatableAssetImage>;
  aspectRatio: number;
  imageConstrain: "fixed" | "filled";
};

type QuickServiceHoursProps = {
  section: {
  ...
  hours: {
    entityHours: YextEntityField<HoursType>;
    fontColor: ThemeColor | undefined;
    hoursStyles: {
...
const resolveImageFieldValue = (value: unknown) => {
  if (
    ...
    return value as HoursSectionImageField["image"]["constantValue"];
  }
...
@media (max-width: 1100px) {
  ...
  .quick-service-hours-section .split-banner-image {
    display: none;
  }
...
const isHoursData = (value: unknown): value is HoursType =>
...
const hasHoursData = (hours?: HoursType) =>
  Boolean(
    hours &&
    Object.values(hours).some((value) => {
      if (!value) return false;
      if (typeof value === "string") return value.trim().length > 0;
      if (value.isClosed) return true;
      return Boolean(value.openIntervals?.length);
    }),
  );

const defaultHoursTextStyles: StyledTextValue = {
  fontFamily: "default",
  fontSize: "default",
  fontWeight: "default",
  fontStyle: "default",
...
const defaultSectionImage: QuickServiceHoursProps["sectionImage"] = {
  image: {
    field: "",
    constantValue: {
      url: hoursImageUrl,
...
const defaultHoursProps: QuickServiceHoursProps = {
  section: {
    backgroundColor: {
    ...
    textStyles: defaultHoursTextStyles,
  },
  heading: {
    text: {
      field: "",
      constantValue: "Dining Hours",
      constantValueEnabled: true,
    },
    styles: defaultHoursTextStyles,
    fontColor: undefined,
  },
  hours: {
    entityHours: {
...
const QuickServiceHoursComponent: PuckComponent<QuickServiceHoursProps> = (
  ...
  const entityHours = isHoursData(resolvedEntityHours)
    ? resolvedEntityHours
    : props.hours.entityHours.constantValue;
  const hasEntityHours = hasHoursData(entityHours);
  const hours =
    props.hours.entityHours.constantValueEnabled || !hasEntityHours
      ? props.hours.entityHours.constantValue
      : entityHours;
  const hoursColor = getThemeColorCssValue(props.hours.fontColor);
  const resolvedEntityImage = resolveComponentData(
  ...
  const alignment =
    props.hours.hoursStyles.alignment === "items-center"
      ? "center"
      : props.hours.hoursStyles.alignment === "items-end"
        ? "flex-end"
        : "flex-start";
  ...
  return (
    <AnalyticsScopeProvider
      name={`QuickServiceHours${getAnalyticsScopeHash(props.id)}`}
    >
      <VisibilityWrapper
      ...
      >
        <>
          <style>{QuickServiceHoursStyles}</style>
          <Background
            as="section"
            background={props.section.backgroundColor}
            className="split-banner quick-service-hours-section quick-service-details-flipped-section"
            style={{
              ...sectionSurfaceStyle,
            }}
          >
            ...
            >
              <article
                className="split-banner-text quick-service-details-content quick-service-hours-content"
                style={{
                  ...sectionTextStyles,
                  display: "flex",
