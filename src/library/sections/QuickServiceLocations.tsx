...
const formatNearbyDistance = (
  ...
  const localeUsesKilometers =
    !normalizedLocale.startsWith("en-us") &&
...
const getNearbyDirectionsUrl = (locationData: NearbyLocationDoc) => {
  const latitude = locationData.yextDisplayCoordinate?.latitude;
  const longitude = locationData.yextDisplayCoordinate?.longitude;
...
const defaultHeading: QuickServiceLocationsProps["heading"] = {
  text: {
    field: "",
...
const defaultMap: QuickServiceLocationsProps["map"] = {
  limit: 3,
  radius: 10,
  coordinate: {
...
const defaultDividerColor: ThemeColor = {
...
const defaultLocationsProps: QuickServiceLocationsProps = {
  section: {
    backgroundColor: {
      selectedColor: "white",
...
> = (props) => {
  ...
  const locations = nearbyLocationDocs.map((locationData) => {
    return {
      name: locationData.name ?? "",
      address: locationData.address,
      phone: locationData.mainPhone ?? "",
      hours: locationData.hours as HoursType | undefined,
      timezone:
  ...
  const showLocationsSection = props.puck.isEditing || locations.length > 0;
  ...
  if (!showLocationsSection && !props.puck.isEditing) {
    return <></>;
  }

  const dividerColor = getThemeColorCssValue(props.locations.dividerColor);
  const cardTitleColor = getThemeColorCssValue(props.locations.cardTitleColor);
  const cardSurfaceStyle = props.locations.cardBackgroundColor
    ? getSurfaceColorStyle(props.locations.cardBackgroundColor, streamDocument)
    : undefined;

  return (
    <AnalyticsScopeProvider
      name={`QuickServiceLocations${getAnalyticsScopeHash(props.id)}`}
    >
      <VisibilityWrapper
      ...
      >
        <>
          <style>{QuickServiceLocationsStyles}</style>
          <Background
            as="section"
                ...
                {nearbyLocationsStatus === "pending" ? (
                  ...
                  <div className="quick-service-location-list">
                    {locations.map((location, index) => (
                      <article
                        key={`${toText(location.name, `location-${index}`)}-${index}`}
                        className="quick-service-location-item"
                        style={{
                          ...cardSurfaceStyle,
                          borderBottom:
                      ...
                      >
                        ...
                        {props.locations.showAddress && location.address ? (
                          <div className="quick-service-location-address">
                            <Address
                              address={location.address}
                              showRegion={props.locations.address.showRegion}
                              showCountry={props.locations.address.showCountry}
                            />
                          </div>
                        ) : null}
                        {props.locations.showHours &&
                        location.hours &&
                        location.timezone ? (
                          <div className="quick-service-location-phone">
                            <HoursStatus
                              hours={location.hours}
                              timezone={location.timezone}
                              dayOptions={{
                                weekday:
                                  props.locations.hoursStyles.dayOfWeekFormat,
                              }}
                              timeOptions={{
                                hour12:
                                  props.locations.hoursStyles.timeFormat ===
                                  "12h",
                              }}
                              statusTemplate={(params: StatusParams) => {
                                ...
                                const showDayOfWeek =
                                  props.locations.hoursStyles.showDayNames &&
                                  Boolean(interval) &&
                                  Boolean(time);
                        ...
                        {props.locations.showPhone && location.phone ? (
                          props.locations.phone.includeHyperlink ? (
                            <Link
                              className="quick-service-location-phone"
                              cta={{
                                link: location.phone.replace(/\D/g, ""),
                                linkType: "PHONE",
                              }}
                            >
                              {formatPhoneNumber(
                                location.phone,
                                props.locations.phone.phoneFormat,
                              )}
                            </Link>
                          ) : (
                            <p className="quick-service-location-phone">
                              {formatPhoneNumber(
                                location.phone,
                                props.locations.phone.phoneFormat,
                              )}
                        ...
                        {props.locations.showDistance && location.distance ? (
                          <p className="quick-service-location-distance">
                            {toText(location.distance)}
                          </p>
                        ) : null}
                        {props.locations.showDirections ? (
                          <Link
                            className="quick-service-detail-cta quick-service-location-link"
                            cta={{
                              link: location.resolvedDirectionsUrl,
                              linkType: "URL",
                            }}
                            style={
                              cardSurfaceStyle?.color
                                ? { color: cardSurfaceStyle.color }
                                : undefined
                            }
