...
type OfferingsContent = {
  list: typeof offeringsContentSource.value;
...
const resolveOfferingsItemText = (
  value: OfferingsItem["text"] | TranslatableString | undefined,
  locale: string,
  ...
  const resolvedValue = resolveComponentData(value, locale, streamDocument);
  ...
  if (
    ...
    return value.constantValue.trim();
  }
...
type QuickServiceOfferingsProps = {
  section: {
...
.site-header-btn,.quick-service-featured-cta,
...
const defaultHeading: QuickServiceOfferingsProps["heading"] = {
  data: {
    text: {
      field: "",
      constantValue: "Offerings",
      constantValueEnabled: true,
    },
  },
  fontOptions: {
...
const defaultContent: OfferingsContent = {
  list: offeringsContentSource.defaultValue,
  styles: {
    fontFamily: "default",
    fontSize: "default",
    fontWeight: "default",
...
const defaultSectionImage: QuickServiceOfferingsProps["sectionImage"] = {
  image: {
    field: "",
    constantValue: {
      url: offeringsImageUrl,
      width: 1267,
      height: 1900,
      alternateText: "",
    },
...
const defaultOfferingsProps: QuickServiceOfferingsProps = {
  section: {
...
const QuickServiceOfferingsComponent: PuckComponent<
...
> = (props) => {
  ...
  const resolvedContentItems = offeringsContentSource
    .resolveItems(props.content.list, streamDocument)
    .map((item) => ({
      ...item,
      text: resolveOfferingsItemText(item.text, locale, streamDocument),
      status: resolveOfferingsItemText(item.status, locale, streamDocument),
    }));
  const contentItems = resolvedContentItems.filter(
  ...
  const contentTextStyle: React.CSSProperties = {
    ...
    color: getThemeColorCssValue(props.content.styles.color),
  };
  ...
  const selectedSectionImage = [
    resolvedSectionImage,
    props.sectionImage.image.constantValue,
  ].find(
    (
      image,
    ): image is NonNullable<OfferingsSectionImage["image"]["constantValue"]> =>
      Boolean(
        image &&
        typeof image === "object" &&
        "url" in image &&
  ...
  return (
    <AnalyticsScopeProvider
      name={`QuickServiceOfferings${getAnalyticsScopeHash(props.id)}`}
    >
      <VisibilityWrapper
      ...
      >
        <>
          <style>{QuickServiceOfferingsStyles}</style>
          <Background
            as="section"
            background={props.section.backgroundColor}
            className="quick-service-offerings-section"
            aria-label="Offerings"
            style={{
              ...sectionSurfaceStyle,
            }}
          >
            <div className="quick-service-offerings-header">
              <div className="split-banner-heading quick-service-offerings-main-heading">
                <EntityField
                  displayName="Heading"
            ...
            <div className="quick-service-offerings-wrap">
              <article className="quick-service-offerings-list-panel">
                <EntityField
                  displayName="Offerings"
                  fieldId={props.content.list.field}
                  constantValueEnabled={props.content.list.constantValueEnabled}
                >
                  <ul className="quick-service-offerings-list">
                    {contentItems.map((item, index) => {
                      return (
                        <li key={`${item.text ?? "offering"}-${index}`}>
                          <span
                            className={`quick-service-offerings-icon ${
                              item.status === "x"
                                ? "quick-service-offerings-icon-x"
                                : "quick-service-offerings-icon-check"
                            }`}
                          ...
                          />
                          <span style={contentTextStyle}>{item.text}</span>
                        </li>
                      );
                    })}
                  </ul>
              ...
              {selectedSectionImage ? (
                <article className="quick-service-offerings-image-panel">
                  <EntityField
